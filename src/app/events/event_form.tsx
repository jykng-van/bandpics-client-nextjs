...
import { MoreData } from "./more_data";

export const LiveEventContext = createContext<LiveEvent | null>(null);
export const ImageGroupsContext = createContext<ImageGroup[]>([]);

export const EventForm = ({liveEvent, groups}:{liveEvent?: LiveEvent, groups?: ImageGroup[]}) => {
    ...
    const [extraDataEdit, setExtraDataEdit] = useState<boolean>(false);
    ...
    const getExtraData = ()=>{
        setExtraDataEdit(true);
    }
    const closeExtraData = ()=>{
        setExtraDataEdit(false);
    }

    return(
        <LiveEventContext value={liveEvent || null}>
            <ImageGroupsContext value={groups || []}>
            {session.status == 'authenticated'  && <form onSubmit={handleSubmit}>
                ...
                {extraDataEdit && <MoreData closeCallback={closeExtraData} />}
                <div className="flex flex-row gap-3">
                    <button className="font-bold text-white bg-blue-800 p-2 inline-block rounded-sm" type="button" onClick={getExtraData}>Additional Data</button>
